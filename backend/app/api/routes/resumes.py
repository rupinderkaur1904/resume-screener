...
async def generate_and_save_embedding(resume_id: int, raw_text: str) -> None:
    ...
    try:
        embedding = await run_in_threadpool(embed_text, raw_text)
        async with AsyncSessionLocal() as session:
            resume = await session.get(Resume, resume_id)
            if resume:
                resume.embedding = embedding
                resume.embedding_status = "ready"
                session.add(resume)
...
async def upload_resume(
    ...
    """
    # --- Read in chunks and enforce the size limit before buffering the full file ---
    from app.config import get_settings as _get_settings
    _upload_settings = _get_settings()
    max_bytes = _upload_settings.MAX_UPLOAD_SIZE_MB * 1024 * 1024
    chunks: list[bytes] = []
    total_size = 0
    ...
    while True:
        ...
        if total_size > max_bytes:
            raise HTTPException(
                ...
                detail=f"File is too large. Maximum allowed size is {_upload_settings.MAX_UPLOAD_SIZE_MB} MB.",
            )
        chunks.append(chunk)
    ...
    try:
        ...
        # RuntimeError is raised by our pdf_parser when PyMuPDF is missing.
        # PyMuPDF raises fitz.FileDataError / fitz.EmptyFileError for corrupt PDFs.
        # Catch broadly since fitz may not be importable at module level.
        raise HTTPException(
