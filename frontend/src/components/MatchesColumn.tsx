...
export function MatchesColumn({ resumes, jobs }: { resumes: Resume[]; jobs: Job[] }) {
  ...
  const selectItems = resumes.map((r) => ({ value: String(r.id), label: r.filename }))
  ...
  return (
    <ColumnCard title="Match Results" icon={<Sparkles className="size-4" />}>
      <Select value={selected} onValueChange={(v) => setSelected(v as string)} disabled={!hasResumes} items={selectItems}>
